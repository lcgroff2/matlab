%SwellETDiff.m: Nanoparticle swelling/energy transfer/exciton diffusion
%simulation -LCG 02/03/2014
...
param.r0 = 0.1; %Assume perylene red-like quenchers for now.
%Cubic lattice spacing (nm):
param.latspc = 0.1;
%Number of chromophores:
param.chrom = ((2*param.nprad)/param.latspc)^3-1;
%Number of quenchers:
param.quench = 1;
%polymer lifetime in good solvent (ps):
param.tau0 = 2800;
%Number of excitons to populate in nanoparticle:
param.nex = ((2*param.nprad)/param.latspc)^3-1;
...
steps = round(4*param.tau0/param.dt); %Number of steps in each individual walk
...
for navg = 1:param.navg,
    ...
    % on vectors:
    np.latpts = zeros(2*param.nprad/param.latspc+2, 2*param.nprad/param.latspc+2, 2*param.nprad/param.latspc+2);
    ...
    cnt = 2:(param.nprad/param.latspc)+1;
    np.latpts(cnt, cnt, cnt) = 2; %place one chromophore and one exciton on each lattice point.
    cnt1=cnt(1:param.nprad/param.latspc/2-1);
    cnt2=cnt(param.nprad/param.latspc/2+1:end);
    catcnt = horzcat(cnt1,cnt2);
    np.cposx(1:length(catcnt)) = catcnt;
    np.cposy = np.cposx;
    np.cposz = np.cposx;

    %     while cnt <= param.chrom,
    %         n = 2*param.nprad/param.latspc;
    ...
    %    if param.quench >= 1
    np.qposx = (param.nprad/param.latspc)+1;
    np.qposy = np.qposx;
    np.qposz = np.qposx;
    np.latpts(np.qposx, np.qposy, np.qposz) = -1; %Replace central lattice point with quencher.
    ...
    %Place excitons on chromophores:
    ...
    while cnt3 <= param.nex
        ...
        for curstep = 1:steps,
            ...
            results.dec(curstep) = sum(decayed);
                            ...
                            case 1 %X axis
                                if np.latpts(np.exposx(aliveidx(cnt5))+1, np.exposy(aliveidx(cnt5)), np.exposz(aliveidx(cnt5))) ~= 0
                                    ...
                                    elseif np.latpts(np.exposx(aliveidx(cnt5))+1, np.exposy(aliveidx(cnt5)), np.exposz(aliveidx(cnt5))) == -1 %hopping to a quencher
                                        ...
                                        np.latpts(np.exposx(aliveidx(cnt5)), np.exposy(aliveidx(cnt5)), np.exposz(aliveidx(cnt5))) = np.latpts(np.exposx(aliveidx(cnt5)), np.exposy(aliveidx(cnt5)), np.exposz(alive(cnt5)))-1;
    ...
    %Compare radial distance from quencher at end of simulation to zero order
    %modified bessel function of second kind K_0
    
    decayidx = find(decayed);

    dx = np.exposx(decayidx)-np.qposx;
    dy = np.exposy(decayidx)-np.qposy;
    dz = np.exposz(decayidx)-np.qposz;
    R = sqrt(dx.*dx+dy.*dy+dz.*dz);
    R2 = 1:1:max(R);
    bess = (param.nex.*besselk(0,R2/param.ld))./R2;
