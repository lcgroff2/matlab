...
function im = sifread1st(fnam)

% sifread function, for reading first frame only
...
hdr{6} = fgetl(fid); % original full file name and path
...
imraw=fread(fid,im.hsize*im.vsize,'float32=>uint16');
...
im.image = reshape(imraw,[im.hsize im.vsize 1]);
