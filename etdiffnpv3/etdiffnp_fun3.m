function results=etdiffnp_fun3(parm)

% exdiff_fun3 -- excitons can diffuse, decay, or undergo energy transfer
...
% Usage: (See etmulti.m for example)
...
% diffusion constant
D = ld^2/(6*tau0); %Edited 03/20/2013 by LCG after speaking to JDM.
...
% will use these instead of making histogram of decay times
results.population=zeros([1 nsteps]);
results.time=results.population;

for curstep=1:nsteps,
  t=dt*(curstep-1); % keep track of time
  ...
  % record population and time at beginning of step
  results.population(curstep)=sum(alive);
