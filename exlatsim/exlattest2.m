...
% TEST VERSION -- PLACES EXCITONS AT DEAD CENTER

% JDM March 2014

% note: for LD=10, dx=0.5, running calculation gives Nhist that is proportional to besselk(0,xhist/10)./xhist , which is what is expected. There is some "noise", likely due to the large grid spacing and/or some poisson noise.
% Nhist is the histogram of excitons. Jhist has the Jacobian factor. Thus to plot Nhist, you need to divide it by Jhist: 
% plot(xhist,Nhist./Jhist,xhist,10*besselk(0,xhist/10)./xhist), and adjust the factor in front of the besselk until it fits.

% simulation parameters -- in later versions these will be specified in a control script instead
dx = 0.5 ; % lattice spacing, nm
ld = 3 ; % exciton diffusion length, nm
tau = 3000; % lifetime, in solution
dt = 10; % time step
%diaghop=0; % allow diagonal "hop" motion? (probability should be less). NOT IMPLEMENTED YET
cubesz=40; % 40 nm cube
fillfrac=1; % chromophore filling fraction (1.0 = all sites filled)
R0=0.1; % forster radius
numq=0; % number of quenchers 
numex=10000; % number of excitons (not sure this is enough)
...
% **TESTING** put some extra chromophores in small cube around dead center, so excitons have a reasonable chance of escaping. Also, this avoids the case where we place all excitons at dead center, and there is no chromophore there
gp=round(gridpoints/2);
cmat(gp+[-1 0 1],gp+[-1 0 1],gp+[-1 0 1])=1; % comment this for quenching calculations
...
%% TESTING -- PUT ALL EXCITONS IN THE MIDDLE
disp('TESTING--ALL EXCITONS IN CENTER')
gp=round(gridpoints/2); exi(:)=gp; exj(:)=gp; exk(:)=gp;
...
%etposstore(1,:)=exi(etidxstore)*dx;
%etposstore(2,:)=exj(etidxstore)*dx;
%etposstore(3,:)=exk(etidxstore)*dx;

decposstore=zeros([3 length(decidxstore)]);

decposstore(1,:)=exi(decidxstore)*dx;
decposstore(2,:)=exj(decidxstore)*dx;
decposstore(3,:)=exk(decidxstore)*dx;


% next, convert each XYZ position into an R value
allpos=decposstore;
allR=sqrt( (allpos(1,:)-gp*dx).^2 + (allpos(2,:)-gp*dx).^2 + (allpos(3,:)-gp*dx).^2 );
...
cubeR=sqrt( (XX-gp*dx).^2 + (YY-gp*dx).^2 + (ZZ-gp*dx).^2 );
cubeR=cubeR(:); % reshape into vector--will histogram them, so order doesn't matter
Jhist=hist(cubeR,r);

save test2results.mat

% calculate mean square displacement, averaged over all axes
msdx=mean((allpos(1,:)-gp*dx).^2)
msdy=mean((allpos(2,:)-gp*dx).^2)
msdz=mean((allpos(3,:)-gp*dx).^2)

fprintf('ave mean square displacement per axis: %.4f nm^2\n',(msdx+msdy+msdz)/3);

% plot versus Bessel function
Nnorm=Nhist./Jhist;
Nnorm=Nnorm(2:end); % throw out first point (due to besselk undefeined at R=0).
r=r(2:end);         % "
bess=besselk(0,r/ld)./r; % proper form for diffusion with decay, 3D
Nnorm=Nnorm/Nnorm(1);
bess=bess/bess(1);
