...
DataFile='profEuBead20um.txt'; % don't get the data and irf files
                            % mixed up, or strange results occur.
IRFFile='Beam_Prof1.txt'; % to get more fit results and possibly
                           % better statistics, try using two
                           % different IRF scans (the one before
                           % the sample scan and the one after)

dt=1;                % dwell time in ps (picoseconds per
                          % bin). This should be re-measured
                          % periodically.

SaveFile = [DataFile '.fit.mat']; % where to save fit results.
dran=100:400; % Data range.  Carefully edit to include 100-300 ps
...
% fitinfo.fun{curfun}.lbound=[6];
% fitinfo.fun{curfun}.ubound=[20];
% fitinfo.fun{curfun}.curval=[15];
...
% curfun=curfun+1;
% fitinfo.fun{curfun}.function = 'exp'; % example of exponential
% fitinfo.fun{curfun}.lbound=[0.1];
% fitinfo.fun{curfun}.ubound=[5];
% fitinfo.fun{curfun}.curval=[0.5];
...
fitinfo.fun{curfun}.function = 'kww'; % can be 'exp', 'kww', 'offset',
                                      %'linear', 'hside', or 'irf'
fitinfo.fun{curfun}.lbound=[0.1 .2]; % first value is the tau
...
ReverseMode=0; % set to "1" for reverse-mode data (PIN sent to STOP input on TAC)
