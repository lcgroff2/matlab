...
DataFile=r2; % don't get the data and irf files
                            % mixed up, or strange results occur.
IRFFile='lcgjyf080113-80%-mag-i3.asc'; % to get more fit results and possibly
...
dran=1500:2500; % Data range.  Carefully edit to include 100-300 ps
                ...
                % percent or less.
Rebinlvl=1; % for re-binning data (boxcar), esp. useful for
...
fitinfo.fun{curfun}.lbound=[15];
fitinfo.fun{curfun}.ubound=[1000];
fitinfo.fun{curfun}.curval=[550];
...
% curfun=curfun+1;
% fitinfo.fun{curfun}.function = 'exp'; % example of exponential
% fitinfo.fun{curfun}.lbound=[50];
% fitinfo.fun{curfun}.ubound=[1500];
% fitinfo.fun{curfun}.curval=[150];
...
%                                       %'linear', 'hside', or 'irf'
% fitinfo.fun{curfun}.lbound=[5 .2]; % first value is the tau
%                                      %parameter, second is beta
% fitinfo.fun{curfun}.ubound=[400 1.0]; % first value is the tau
%                                        %parameter, second is beta
% fitinfo.fun{curfun}.curval=[15 .7]; % first value is the tau
%                                      %parameter, second is beta
...
fitinfo.fun{curfun}.lbound=-0.05;
fitinfo.fun{curfun}.ubound=0.25;
fitinfo.fun{curfun}.curval=0.2;
...
% fitinfo.fun{curfun}.lbound=1;
% fitinfo.fun{curfun}.ubound=5;
% fitinfo.fun{curfun}.curval=2;
...
figure(1);
plot(DataX,DataY./max(DataY),IRFx,IRFy(dran)./max(IRFy(dran)))
