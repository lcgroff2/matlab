...
dran=1550:2200; % Data range.  Carefully edit to include 100-300 ps
...
D_t = A0*DataY0-A90*DataY90; %D(t) = Ipara-g*Iperp
S_t = A0*DataY0+2*A90*DataY90; %S(t) = Ipara+2*g*Iperp
S_t = S_t+.005*max(S_t);
r=D_t./(3*DataY55); % factor of 3 in denominator, since I at magic angle = Ipara + 2*Iperp
r2=D_t./S_t;
...
xlabel('Time (ps)')
ylabel('Anisotropy')
legend('(I0-I90)/(3*I55)','(I0-I90)/(I0+2*I90)')

figure(2)
plot(DataX,IRFy/max(IRFy)*max(DataY55),DataX,DataY0,DataX,DataY55,DataX,DataY90)
axis tight
...
figure(3)
plot(DataX,S_t,DataX,D_t,DataX,3*DataY55)
axis tight
legend('S(t)','D(t)','3*I_5_5(t)')
xlabel('Time (ps)')
ylabel('Intensity (au)')
