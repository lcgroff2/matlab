...
numits=6; % 4 iterations, reducing by a factor of itfactor
itfactor=5;
betastepsz=0.0005;
t0stepsz=0.1; 
taustepsz=0.2;
...
for cnt=1:length(fitinfo.fun),
    if fitinfo.fun{cnt}.function=='exp'
        if fitinfo.fun{cnt}.lbound(1) < fitinfo.fun{cnt}.ubound(1) % if fixed, then don't include
            parmidx{end+1}=[cnt 1];
    ...
    if fitinfo.fun{cnt}.function=='kww'
        if fitinfo.fun{cnt}.lbound(1) < fitinfo.fun{cnt}.ubound(1)
...
%  outer loop - rerun main loop several times
for curit=1:numits % rerun "numits" times
    ...
    while 1 % main while loop
        ...
        for cnt=1:numeval
            ...
            fittmp=fitinfo;
            for curvar=1:numvar
                % make changes to each "curval" as indicated by current move "curmove"
                if curvar==1 % for t0 parameter
                    fittmp.t0curval=fittmp.t0curval+curmove(1)*parmstepsz(1);
                else
                    parmtmp=fittmp.fun{parmidx{curvar}(1)}.curval(parmidx{curvar}(2));
                    fittmp.fun{parmidx{curvar}(1)}.curval(parmidx{curvar}(2))=parmtmp+curmove(curvar)*parmstepsz(curvar);
                end
            end
        ...
        % find minimum sqerr
        [val,idx]=min(sqerrs);
        % part 4: if min is at the origin , then "break", thus moving to smaller step size
        if idx==centerpoint
            fprintf('minimum reached after %i iterations\n',iits);
            break;
        end
        ...
        % part 4.15: keep stepping along same direction until sqerr no longer decreases,
        % this hack assumes that more often than not, future moves along the same direction
        % will improve sqerr. This assumption should reduce computational cost.
        sqerrprev=val;
        curmove=moves(idx,:);
        while 1
            fittmp=fitprev;
            ...
            [sqerror,yfit,amp]=kwwfit_fun(fittmp);
            if sqerror<sqerrprev
                fitprev=fittmp;
                newfitinfo=fittmp;
                sqerrprev=sqerror;
            else
                break; % no improvement, so break out of while loop
            end
             % note: make sure not to exceed bounds
             %disp('possible move found');
             %newfitinfo=fitinfoarray(idx); % candidate fitinfo
             brkflag=0; % flag for breaking out if stalled against a bound
        ...
        end % end while (straight stepping hack)
        ...
        % break out of while loop if brkflag set
        if brkflag==1
            break;
        end
        ...
        if cputime-starttime>30
            fprintf('time-out after 30 seconds and %i iterations\n',iits);
            break;
        end
        %swap in new values for fitinfo
        fitinfo=newfitinfo;
        iits=iits+1; % inner iterations
    end % end while
        
    % decrease step sizes
    parmstepsz=parmstepsz/itfactor;
end % end for
